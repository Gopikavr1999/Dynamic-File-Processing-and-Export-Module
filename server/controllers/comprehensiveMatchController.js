...
const comprehensiveMatchController = async (allDocuments, contentValues, backend) => {
    try {
        ...
        for (const document of allDocuments) {
            ...
            if (newRelatedTo.length > 0) {
                // Check if the document already has a "Comprehensive Unique ID Connection" relation
                let updated = false;
                for (let rel of relation) {
                    if (rel.relationType === "Comprehensive Unique ID Connection") {
                ...
                if (!updated && newRelatedTo.length > 0) {
                    ...
                    relation.push({
                        relationType: "Comprehensive Unique ID Connection",
